    ...
    <section className="activity-page" aria-labelledby="activity-title">
      ...
      <section className="timeline-section" aria-labelledby="timeline-title">
        ...
        <form className="filter-grid" onSubmit={(event) => { event.preventDefault(); void load(true); }}>
          ...
          <label>{t("activity.eventType")}<select value={eventType} onChange={(event) => setEventType(event.currentTarget.value as ActivityRequest["eventType"] | "")}><option value="">{t("activity.all")}</option><option value="Income">{t("event.income")}</option><option value="Expense">{t("event.expense")}</option><option value="Adjustment">{t("event.adjustment")}</option><option value="Transfer">{t("event.transfer")}</option><option value="CurrencyExchange">{t("event.currencyExchange")}</option><option value="Reversal">{t("event.reversal")}</option></select></label>
...
export function ActivityDetails({ item, accounts, categories, t, canChange, onClose, onRevise, onReverse }: { item: ActivityItem; accounts: CatalogRecord[]; categories: CatalogRecord[]; t: (key: Parameters<typeof translate>[1]) => string; canChange: boolean; onClose: () => void; onRevise: () => void; onReverse: () => void }) {
  ...
  return <aside className="detail-panel" aria-labelledby="detail-title"><div className="detail-header"><div><p className="eyebrow">{t("activity.detailEyebrow")}</p><h2 id="detail-title">{displayEventType(item.eventType, t)}</h2></div><button type="button" className="secondary" onClick={onClose}>{t("activity.closeDetails")}</button></div><dl className="detail-grid"><div><dt>{t("field.date")}</dt><dd>{item.effectiveDate}</dd></div><div><dt>{t("activity.revision")}</dt><dd>r{item.revision}</dd></div><div><dt>{t("activity.account")}</dt><dd>{accountName(item.content.accountId ?? item.content.fromAccountId)}</dd></div><div><dt>{t("activity.toAccount")}</dt><dd>{accountName(item.content.toAccountId)}</dd></div><div><dt>{t("activity.amount")}</dt><dd>{item.content.amount ?? "—"}</dd></div><div><dt>{t("activity.toAmount")}</dt><dd>{item.content.toAmount ?? "—"}</dd></div><div><dt>{t("activity.category")}</dt><dd>{categoryName}</dd></div><div><dt>{t("field.semanticRole")}</dt><dd>{item.content.semanticRole}</dd></div><div><dt>{t("activity.merchant")}</dt><dd>{item.content.merchant ?? "—"}</dd></div><div><dt>{t("activity.note")}</dt><dd>{item.content.note ?? "—"}</dd></div></dl><h3>{t("activity.postings")}</h3><PostingTable postings={item.postings} t={t} /><h3>{t("activity.fxResolution")}</h3>{item.fxResolutions.length === 0 ? <p className="empty-state">{t("activity.noFx")}</p> : <ul className="resolution-list">{item.fxResolutions.map((resolution) => <li key={`${resolution.purpose}-${resolution.currency}`}><strong>{resolution.currency} → {resolution.baseCurrency}</strong><span>{resolution.finalRate}</span><small>{resolution.overrideValue ? t("activity.manualOverride") : t("activity.automaticRate")} · {resolution.targetDate} · {resolution.calculationVersion}</small></li>)}</ul>}<h3>{t("activity.history")}</h3>{relations.length === 0 ? <p className="empty-state">{t("activity.noRelations")}</p> : <ul className="relation-list">{relations.map(([kind, value]) => <li key={kind}><strong>{kind}</strong><code>{value}</code></li>)}</ul>}<p className="audit-line"><strong>{t("activity.audit")}</strong> {item.audit.action} · {item.audit.occurredAtUtc}{item.audit.reason ? ` · ${item.audit.reason}` : ""}</p>{canChange ? <div className="form-actions"><button type="button" onClick={onRevise}>{t("activity.revise")}</button><button type="button" className="danger" onClick={onReverse}>{t("activity.reverse")}</button></div> : <p className="empty-state">{t("activity.immutableHistory")}</p>}</aside>;
}

...
>
> 适用范围：从已知 `多币种个人账本v1.3.0.xlsx` 模板进行一次性初始迁移。
...
1. 在桌面 Excel 中对迁移副本执行全量重算并保存。
2. 记录副本哈希、大小、修改时间、导入器版本和目标 schema 版本。
3. 确认本位币、金额精度、成本基础、修订语义和支出分析 ADR。
4. 逐账户/组合选择并记录完整历史重建或明确 cut-over。
5. 准备与真实数据结构等价的脱敏黄金 fixture。

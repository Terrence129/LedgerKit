...
>
> 源文件 SHA-256：`20CAFF41D7E5D08F71591CF8206DB015905BEAD40BC5AFEFD25008EE648D5820`（用户于 2026-09-03 确认为新的权威迁移源）
>
...
- 初始调查时，源文件所在目录不是 Git 仓库，且其中没有项目 `AGENTS.md`、`README`、源码、架构文档或既有 `docs/`。
- 本计划以用户明确选择的仓库外私人文件 `多币种个人账本v1.3.0.xlsx` 为权威迁移源和最新产品参考；通过文件名和本计划顶部记录的 SHA-256 校验源文件身份。v1.2.0 仅用于确认版本差异。
- 工作区中出现的其他版本、输出文件和临时分析目录不自动成为需求或迁移源。
- 权威源文件位于仓库外的私人 OneDrive 目录；当前规范工作区把其绝对路径保存在 Git 忽略的 `docs/local/private-source-workbook.md`，供获得用户授权的 agent 定位原件。该路径不写入 tracked 仓库、公开文档、日志、诊断包或 fixture。源目录位于 OneDrive 不意味着生产 SQLite 活库可以安全地直接置于同步目录。
...
| 界面语言 | 仅支持简体中文（`zh-CN`）和英文（`en-US`）；设置页可即时切换 | 首次启动跟随 Windows 显示语言，非中文环境回退英文；用户选择持久化，重启后保持；两套资源键完整一致 |
...
5. **设置与数据**：界面语言、本位币、机构、分类、汇率/价格参考数据、导入导出、备份恢复和诊断。
...
| 实体/契约 | 类型 | 关键字段/约束 |
|---|---|---|
| AppSettings | 源数据 | base_currency、locale（仅 `zh-CN`/`en-US`）、valuation_defaults；首次默认由 Windows 显示语言解析，用户选择后持久化；出现首条依赖记录后 base_currency 不可原地修改 |
...
- 投影删除后必须能从源事件完全重建。
- P0 支出分析通过具名只读 `get_expense_analysis(start_date,end_date)` 在同一 SQLite read snapshot 中产生规范结果。生产事实扫描在任务 07 的 10 万事件门禁中失败后，项目所有者已接受 ADR-0015；当前水位从可重建日聚合投影读取，历史水位仍从 `BusinessEvent`、类型明细、`LedgerPosting` 和冻结 `FxResolution` 重放。投影不成为第二事实源。
...
- UI component tests：复杂表单验证、影响预览、支出图表/表格共享结果、空态、键盘下钻，以及 `zh-CN`/`en-US` 资源键完整性和切换后重渲染。
- 少量 E2E：首次导入、五类录入、冲正、估值/检查、支出改期与图表下钻、备份恢复，以及语言切换持久化和重启恢复。
- Packaging/privacy tests：干净 Windows 安装/升级/卸载、断网运行、日志脱敏，并分别以中文和英文启动完成 smoke test。
...
- 所有列表分页/虚拟化，查询有索引和上限。
- 余额、持仓等高频汇总使用可重建投影；支出分析按 ADR-0015 使用版本化可重建日聚合层，权威事实仍只有类型化事件和 posting。
...
- 1.0 界面只支持简体中文（`zh-CN`）和英文（`en-US`），不得在未评审资源完整性、字体、布局和维护成本前扩展第三种语言。
- 首次启动读取 Windows 显示语言：中文语言环境选择 `zh-CN`，其余或无法识别时回退 `en-US`；用户可在“设置与数据”中即时切换，应用自有窗口无需重启，选择持久化并在下次启动恢复。操作系统原生文件选择器等平台窗口仍跟随 Windows 语言。
- 所有应用自有导航、表单、校验、空态、加载态、错误、确认对话框、表头、系统桶标签、导入/导出、备份/恢复和数据质量说明必须使用资源键，不得在组件中散落中英文硬编码。
- 两套资源目录必须键集合一致；缺失键在检查/测试阶段失败，运行时安全回退 `en-US`，不得显示空白或资源键给用户。切换语言时同步更新文档/窗口语言属性和可访问名称。
- 日期、数字和货币显示使用当前 UI locale formatter；权威 DecimalString、ISO 风格日期、币种代码、稳定 ID、错误码、policy/calculation version 和 canonical hash 不随语言变化。
- 用户输入的机构、账户、分类、商户、备注和附件名称保持原文，应用不得自动翻译或为了切换语言改写业务数据。
...
| M1 轻量技术样机 | 1.5–2 周 | Tauri/.NET 同夹具样机；权威 Core；受控 SQLite migration；Excel 只读解析；11 行原生横条+同源表格；最小备份；权限负测；双口径体积/内存/启动基准；最终骨架建立 `zh-CN`/`en-US` 资源入口 | 用户接受一次性轻量基线；支出分析新增生产依赖为 0并达到页面增量预算；最终骨架双语资源检查和切换 smoke test 通过；否则换栈 |
| M2 基础与现金纵切 | 1.5–2 周 | schema、目录数据、双语首次设置/设置页、收支/调整/调拨/换汇、汇率、活动时间线、现金投影、ExpenseAnalysisQuery 契约/索引 | 现金与支出黄金用例全绿；语言切换持久化；现金投影可重建，10 万事件支出 query 达门禁 |
...
- 领域规则和舍入规范。
- `ExpenseAnalysisQuery` 契约、版本化可重建日聚合查询与 view model；投影选择由 Accepted ADR-0015 约束。
...
- Windows 桌面安装包。
- 简体中文/英文资源目录、语言切换与 locale formatter。
- 本地数据库及备份恢复工具。
...
- 键盘、错误提示和非颜色状态表达通过检查。
- `zh-CN`/`en-US` 资源键集合一致，核心 E2E 在两种语言下通过，语言切换与重启恢复符合计划；切换不得改变任何权威业务数据或 canonical hash。
- 文档、ADR 和用户可见说明同步更新。

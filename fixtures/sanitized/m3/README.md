...
- `cash-import-valid.xlsx` covers the eight supported cash-migration sheets, a formula-backed authoritative input with a cached value, derived/status/display formulas, explicit per-account cut-over policies and lifecycle state, FX, income, expense, fee, transfer, and exchange rows.
